...
import hr.lib.cache.constant.TimeUnit;
...
public class CacheManager extends CacheBase {
	...
	private void initCacheManager() {
		...
		long periodChecking;
		try {
			String periodCheckingStr = ResourceUtil.getApplicationConfigurationFromFile("periodChecking");
			periodChecking = Long.parseLong(periodCheckingStr);
		...
		final int secondCondition = (int) periodChecking/1000;
		timer = new Timer();
			...
			public void run() {
				...
				try {
					for(Map.Entry<String, Object> cacheObjectMap : hashMap.entrySet()) {
						...
						if (cNow.get(Calendar.HOUR_OF_DAY) == 1 && cNow.get(Calendar.MINUTE) == 0 &&
								cNow.get(Calendar.SECOND) < secondCondition) {
							if (cache.getIsSync()) {
								if (cache.loadAll()) {
